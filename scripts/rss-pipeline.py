#!/usr/bin/env python3
"""
CanadaScanada RSS Pipeline v2.1 - parallel fetch with hard timeouts

Lives in /scripts/. Reads project data from the repo root.
"""
...
# Script lives in /scripts/; ROOT is the repo root one level up.
ROOT = Path(__file__).resolve().parent.parent
FEED_MAP_PATH       = ROOT / "meta" / "feed-map.json"
KEYWORD_FILTER_PATH = ROOT / "meta" / "keyword-filter.json"
...
def fetch_one(feed_config, kf, known_urls):
    ...
    try:
        ...
        status = f"OK {len(entries)} entries"
...
def main():
    ...
    print(f"CanadaScanada RSS Pipeline v2.1 - {ts}")
        ...
        try:
            for future in as_completed(futures, timeout=GLOBAL_TIMEOUT_SECONDS):
                ...
                try:
                    ...
                    print(f"  ERR {fc['feed_id']}: thread error - {e}")
